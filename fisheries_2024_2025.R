...
#FISHERY MONITORING DATA 2024-2025
#AUTOMATED SCRIPT TO CALCULATE CPUE, LPUE AND FISHING PRESSURE
...
if (!requireNamespace("tidyr", quietly = TRUE)) {
  install.packages("tidyr", dependencies = TRUE, type = "binary")
}
library(tidyr)
...
#4. CALCULATE CPUE
#Calculate CPUE for each species by dividing catch by total effort per gear type
...
species_catch_per_date_and_gear <- expanded_data %>%
  filter(Species_cn == "coney", `Partial/Whole` == "Whole") %>%  #Filter for species of interest and whole counts
  group_by(`Date (DD/MM/YYYY)`, Fishing_gear) %>% 
  summarise(
...
grid.arrange(grobs = all_species_cpue_plots, ncol = 3) 


# Loop through each species to create and print CPUE plots
for (species in species_list) {      
...
# Combine all plots into a single plot grid
grid.arrange(grobs = all_species_lpue_plots, ncol = 2) 




# LPUE GLM - Marketable lobsters -------------------------------------------------------
...
ggplot(month_counts, aes(x = Month, y = n, fill = Month)) +  
  ...
  labs(title = "Number lobster trips per month", x = "Month", y = "No trips") +  
  theme_minimal() +  
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  
...
#Loop through each row in lob_SI_shorts
for(i in 1:nrow(lob_SI_shorts)) {
  
  areas <- unlist(strsplit(as.character(lob_SI_shorts$Fishing_area[i]), split = "/"))
  
  if(length(areas) > 1) {
    total_lobsters <- lob_SI_shorts$Lobster_no[i]
    lobsters_per_area <- floor(total_lobsters / length(areas))
    remainder <- total_lobsters %% length(areas)
    
    for(j in 1:length(areas)) {
      new_row <- lob_SI_shorts[i, ]  
      new_row$Fishing_area <- areas[j]  
...
ggplot(data = predat) + 
  geom_pointrange(aes(x = Fishing_area, y = fit, ymin = lb, ymax = ub), col = "blue") + 
  xlab("Fishing Area") +  # Corrected the label for the x-axis
  ylab("Modelled Number of Undersized Lobsters per Month") + 
  ggtitle("Modelled Boat Effect") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x labels for better readability
...
for(i in 1:nrow(lob_SI_berried)) {
  ...
  if(length(areas) > 1) {
    total_lobsters <- lob_SI_berried$Lobster_no[i]
    lobsters_per_area <- floor(total_lobsters / length(areas))
    remainder <- total_lobsters %% length(areas)
    
    for(j in 1:length(areas)) {
      new_row <- lob_SI_berried[i, ]  
      new_row$Fishing_area <- areas[j]  
...
# M2_berried: Negative Binomial model
# Full model
M2_berried <- glm.nb(Lobster_no ~ factor(Month) + factor(Boat_name) + factor(Fishing_area) + logTraps, 
                     data = lob_SI_lp)

summary(M2_berried) # AIC output
...
grid.arrange(boat_effort_si, boat_effort_onboard, nrow=1)



# Redfish - lobster basic stats -------------------------------------------


#LOBSTER STATISTICS
mean_lobsters_per_trip <- data %>% 
  filter(Species_cn == "caribbean spiny lobster") %>%       # keep only the target species
  group_by(Trip_ID) %>%                                      # one group per trip
  summarise(lobsters_this_trip = n(), .groups = "drop") %>%  # count rows = lobsters
  summarise(mean_lobsters = mean(lobsters_this_trip))

mean_csl_length <- data %>%                 # your data frame
  filter(Species_cn == "caribbean spiny lobster") %>% 
  summarise(mean_length = mean(Length, na.rm = TRUE))

mean_csl_m_length <- data %>%                 # your data frame
  filter(Species_cn == "caribbean spiny lobster", Sex == "M") %>% 
  summarise(mean_length = mean(Length, na.rm = TRUE))

mean_csl_f_length <- data %>%                 # your data frame
  filter(Species_cn == "caribbean spiny lobster", Sex == "F") %>% 
  summarise(mean_length = mean(Length, na.rm = TRUE))
...
avg_lbs_per_trap <- lob_SI_rp %>%
  summarise(
    avg_traps = mean(Traps_no, na.rm = TRUE),
    avg_redfish = mean(Redfish_lbs, na.rm = TRUE)
  ) %>%
  mutate(avg_lbs_per_trap = avg_redfish / avg_traps)
